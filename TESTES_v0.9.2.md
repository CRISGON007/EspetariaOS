# Checklist de testes — EspetariaOS v0.9.2
...
## Correções de ambiente da v0.9.2
...
## Gestão administrativa v0.9.2
- [ ] Clientes listados e pesquisados por nome/telefone.
- [ ] Totais por cliente exibidos.
- [ ] Auditoria com filtros.
- [ ] Backup automático diário criado e auditado.

## Backup automático v0.9.2
- [ ] Backup é verificado imediatamente na inicialização.
- [ ] Um segundo backup não é criado no mesmo dia.
- [ ] Apenas os 30 backups mais recentes são mantidos.
- [ ] A limpeza aparece na auditoria.

## Estoque v0.9.2
- [ ] Controle opcional por produto.
- [ ] Entradas, perdas e ajustes.
- [ ] Baixa automática no pedido.
- [ ] Bloqueio com saldo insuficiente.
- [ ] Devolução no cancelamento.
- [ ] Alertas de estoque baixo e zerado.

## Cadastro de estoque pelo administrador
- [ ] Ao ativar controle de estoque, os campos aparecem.
- [ ] Quantidade disponível e quantidade mínima são obrigatórias.
- [ ] Valores negativos são rejeitados.
- [ ] Os valores são salvos e reaparecem na edição.
- [ ] Ao atingir o mínimo, aparece alerta de estoque baixo.


## Estabilização dos testes — v0.9.2

- [ ] `./testar.sh` conclui sem `NameError`.
- [ ] Cadastro de produto com estoque é validado.
- [ ] Entrada de estoque é validada.
- [ ] Histórico de movimentação é validado.
- [ ] Pedido e rastreamento são validados.
- [ ] Histórico de status é validado.
- [ ] Retenção de backups mantém somente a quantidade configurada.

## Financeiro v0.9.2
- [ ] Despesas podem ser cadastradas e excluídas.
- [ ] Filtros por período, categoria, pagamento e texto funcionam.
- [ ] Receita, despesas e saldo são calculados.
- [ ] Resumo por categoria aparece.
- [ ] Atualização em tempo real funciona.
- [ ] `publicar_git.sh` executa testes antes de publicar.


## Correção da exclusão de despesas

- [ ] A aplicação é importada sem `AssertionError`.
- [ ] A rota DELETE de despesas retorna HTTP 200.
- [ ] A resposta da exclusão contém `{"ok": true}`.
- [ ] A despesa é removida da listagem.
- [ ] O financeiro é atualizado após a exclusão.


## Relatórios v0.9.2

- [ ] Período padrão de 30 dias é preenchido.
- [ ] Receita, despesas e resultado líquido são exibidos.
- [ ] Ticket médio é exibido.
- [ ] Taxa de cancelamento é exibida.
- [ ] Tempo médio de preparo é exibido.
- [ ] Receita diária aparece no gráfico.
- [ ] Produtos mais vendidos aparecem no ranking.
- [ ] Formas de pagamento aparecem no ranking.
- [ ] Status dos pedidos aparecem no ranking.
- [ ] Melhores clientes aparecem no relatório.
...
- [ ] O tempo continua contando para pedidos Recebidos.
- [ ] O tempo continua contando para pedidos Em preparo.
- [ ] O tempo continua contando para pedidos Prontos.
- [ ] O tempo para quando o pedido é Entregue.
- [ ] O tempo para quando o pedido é Cancelado.
- [ ] O tempo total nas Consultas de Vendas não aumenta após a entrega.
- [ ] O filtro de Auditoria contém Pedido entregue sem pagamento confirmado.
- [ ] O filtro de Auditoria contém Sangria e Suprimento.
- [ ] O filtro de Auditoria contém todas as ações do mapa PT_BR.auditAction.


## Confirmação de pagamento com caixa

- [ ] Com o caixa fechado, Confirmar pagamento exibe aviso.
- [ ] O aviso oferece a opção de abrir o caixa.
- [ ] Ao cancelar a abertura, o pagamento não é confirmado.
- [ ] Ao informar o valor inicial, o caixa é aberto.
- [ ] A escolha da forma de pagamento aparece somente após o caixa abrir.
- [ ] O pagamento é confirmado somente com caixa aberto.
- [ ] O painel de caixa é atualizado após a confirmação.
